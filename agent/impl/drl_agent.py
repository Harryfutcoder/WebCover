...
from utils import (
    instantiate_class_by_module_and_class_name,
    safe_console_text,
    read_reward_clip_abs,
    clip_reward_value,
)
...
class DRLagent(agent.Agent):
    def __init__(self, params, transformer=None):
        ...
        raw_capacity = os.environ.get(
            "WEBTEST_REPLAY_BUFFER_CAPACITY",
            str(params.get("replay_buffer_capacity", 10000)),
        )
        try:
            replay_capacity = int(raw_capacity)
        except (TypeError, ValueError):
            replay_capacity = 10000
        if replay_capacity <= 0:
            replay_capacity = 10000
        self.replay_buffer = PrioritizedReplayBuffer(capacity=replay_capacity)
        self.reward_clip_abs = read_reward_clip_abs(params.get("reward_clip_abs", 5.0))
        ...
        self.reward_function = os.environ.get(
            "WEBTEST_DRL_REWARD_FUNCTION",
        ...
        # 仅观测与日志，不参与训练
        # OBSERVATION_MG_FULL_HISTORY=1：边际增益用全量历史（短测、相关系数更稳）
        ...
        print(
            "DRL config:",
            f"replay_capacity={replay_capacity},",
            f"reward_clip_abs={self.reward_clip_abs if self.reward_clip_abs is not None else 'disabled'}",
        )
        self._obs_step_counter = 0

    def _max_state_similarity(self, web_state, update_seen=True):
        """Compute max similarity against visited states and whether state is newly discovered."""
        is_new_state = not self.state_appear_list.__contains__(web_state)
        if not is_new_state:
            max_sim = 1.0
        else:
            max_sim = -1.0
            for temp_state in self.state_list:
                if (
                    isinstance(temp_state, OutOfDomainState)
                    or isinstance(temp_state, ActionExecuteFailedState)
                    or isinstance(temp_state, SameUrlState)
                ):
                    continue
        ...
        if max_sim < 0:
            max_sim = 0.0
        if update_seen and is_new_state:
            self.state_appear_list.append(web_state)
        return max_sim, is_new_state

    def get_reward(self, web_state):
        if self.reward_function == "STATE_ONLY":
            if not isinstance(web_state, ActionSetWithExecutionTimesState):
                return -1.0
            max_sim, _ = self._max_state_similarity(web_state)
            return max(0.0, 1.0 - max_sim)

        if self.reward_function == "STATE_BINARY_1":
            if not isinstance(web_state, ActionSetWithExecutionTimesState):
                return -1.0
            _, is_new_state = self._max_state_similarity(web_state)
            return 1.0 if is_new_state else 0.0

        if self.reward_function == "STATE_BINARY_100":
            if not isinstance(web_state, ActionSetWithExecutionTimesState):
                return -1.0
            _, is_new_state = self._max_state_similarity(web_state)
            return 100.0 if is_new_state else 0.0

        if self.reward_function == "STATE_COUNT":
            if not isinstance(web_state, ActionSetWithExecutionTimesState):
                return -1.0
            self._max_state_similarity(web_state)
            self.state_visit_count[web_state] += 1
            return 1.0 / math.sqrt(self.state_visit_count[web_state])
    ...
    def get_action(self, web_state: WebState, html: str) -> WebAction:
        ...
        if (
            ...
            and not self.stop_update
        ):
            step_reward = float(self.update(html, web_state))
        ...
        history_for_mg = list(self.state_list[:-1]) if len(self.state_list) > 0 else []
        marg_count = max(1, self.action_count[action_index])
        ...
        self.observer.record_step(
            ...
            max_q_value=max_val if not stop_update_round else 0.0,
    ...
    def learn(self):
        ...
        if self.learn_step_count % self.update_network_interval != 0:
            ...
            # 从经验回放池中采样一批经验
        tensors, _best_tensors, rewards, next_states, htmls, dones, weights, indices = self.replay_buffer.sample(
            self.batch_size
        )
    ...
    def update(self, html: str, web_state: WebState):
        ...
        tensor.unsqueeze_(0)
        self.replay_buffer.push(tensor, tensor, reward, web_state, html, done)
        ...
        if (isinstance(web_state, ActionSetWithExecutionTimesState) and
            ...
            if self.same_url_count >= self.MAX_SAME_URL_COUNT:
                for i in range(self.same_url_count):
                    ...
                    ft = self.fail_tensor_list[i]
                    self.replay_buffer.push(
                        ft, ft, self.fail_reward_list[i], self.fail_web_state_list[i], self.fail_html_list[i], False
                    )
