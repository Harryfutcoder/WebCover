...
_repo_root = os.environ.get("WEBTEST_REPO_ROOT", "").strip()
...
def makeGraph(stateMap,output):
    ...
    with open(os.path.join(output, "data.js"), "w", encoding="utf-8") as jsonwriter:
        C = []
...
def append_native_trace(path, record):
    try:
        with open(path, "a", encoding="utf-8") as trace_writer:
            trace_writer.write(json.dumps(record, sort_keys=True, ensure_ascii=False) + "\n")
    except Exception:
        pass
...
def q_learning(env, num_episodes,sleep=0,matrix=None,statemap=None,_epsilon=0.2,
    ...
    canonical_recorder = (
        recorder_from_env(
            default_algorithm="qexplore",
            default_seed=os.environ.get("WEBTEST_EXTERNAL_CANONICAL_SEED", ""),
            default_profile="qexplore-1agent",
        )
    ...
    native_trace_path = "qexplore_native_trace.jsonl"
    #-=-=-==-=-=-=-=-=-=-=-
    if timebound:
        def some_task():
            global CLOSE
    ...
    #for i_episode in tqdm(range(num_episodes)):
        ...
        if canonical_recorder is not None:
            try:
                canonical_recorder.observe_state(env.website.current_url, state_actions)
            except Exception:
                pass
        prev_action = ""
        actionlist = []
        ...
        for t in itertools.count():
            ...
            with open(os.path.join(output, state + ".html"), "w", encoding="utf-8") as htmlwriter:
            ...
            try:
                canonical_source_url = env.website.current_url
            except Exception:
                canonical_source_url = ""
            canonical_source_actions = list(state_actions)
            ...
            if reward==-999999999999:
                ...
                with open(os.path.join(output, next_state + ".html"), "w", encoding="utf-8") as htmlwriter:
                    htmlwriter.write(env.website.page_source)

            target_url = ""
            try:
                target_url = env.website.current_url
            except Exception:
                target_url = ""
            append_native_trace(native_trace_path, {
                "action": action,
                "action_count": len(state_actions),
                "done": bool(done),
                "episode": i_episode,
                "next_action_count": len(next_state_actions),
            ...
            if canonical_recorder is not None and elem is not None and status:
                try:
                    canonical_recorder.record_transition(
                        source_url=canonical_source_url,
                        source_actions=canonical_source_actions,
                        selected_action=action,
                        target_url=env.website.current_url,
                        target_actions=next_state_actions,
                    )
                except Exception:
                    pass
...
def main():
    ...
    parser.add_argument("--ignore-gooey", action="store_true", help="Compatibility flag for CLI-only runs")
